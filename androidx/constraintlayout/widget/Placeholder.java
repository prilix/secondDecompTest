...
public class Placeholder extends View {
    ...
    private void init(AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attributeSet, C0219R.styleable.ConstraintLayout_placeholder);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0219R.styleable.ConstraintLayout_placeholder_content) {
                    this.mContentId = obtainStyledAttributes.getResourceId(index, this.mContentId);
                } else if (index == C0219R.styleable.ConstraintLayout_placeholder_placeholder_emptyVisibility) {
    ...
    public void onDraw(Canvas canvas) {
        if (isInEditMode()) {
            canvas.drawRGB(C1662R2.attr.carousel_previousState, C1662R2.attr.carousel_previousState, C1662R2.attr.carousel_previousState);
            Paint paint = new Paint();
            paint.setARGB(255, C1662R2.attr.cardCornerRadius, C1662R2.attr.cardCornerRadius, C1662R2.attr.cardCornerRadius);
