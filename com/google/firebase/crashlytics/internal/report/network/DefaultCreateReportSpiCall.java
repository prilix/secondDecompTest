...
public class DefaultCreateReportSpiCall extends AbstractSpiCall implements CreateReportSpiCall {
    ...
    public boolean invoke(CreateReportRequest createReportRequest, boolean z) {
        if (z) {
            ...
            logger.mo24918d("Sending report to: " + getUrl());
            try {
                HttpResponse execute = applyMultipartDataTo.execute();
                int code = execute.code();
                Logger logger2 = Logger.getLogger();
                logger2.mo24918d("Create report request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
                Logger logger3 = Logger.getLogger();
                logger3.mo24918d("Result was: " + code);
                return ResponseParser.parse(code) == 0;
            } catch (IOException e) {
                Logger.getLogger().mo24921e("Create report HTTP request failed.", e);
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest httpRequest, Report report) {
        ...
        if (report.getFiles().length == 1) {
            Logger.getLogger().mo24918d("Adding single file " + report.getFileName() + " to report " + report.getIdentifier());
            return part.part(FILE_PARAM, report.getFileName(), FILE_CONTENT_TYPE, report.getFile());
        }
        int i = 0;
        for (File file : report.getFiles()) {
            Logger.getLogger().mo24918d("Adding file " + file.getName() + " to report " + report.getIdentifier());
