...
abstract class AbstractAppSpiCall extends AbstractSpiCall implements AppSpiCall {
    ...
    public boolean invoke(AppRequestData appRequestData, boolean z) {
        if (z) {
            ...
            logger.mo24918d("Sending app info to " + getUrl());
            try {
                HttpResponse execute = applyMultipartDataTo.execute();
                int code = execute.code();
                String str = Constants.HttpMethods.POST.equalsIgnoreCase(applyMultipartDataTo.method()) ? "Create" : "Update";
                Logger logger2 = Logger.getLogger();
                logger2.mo24918d(str + " app request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
                Logger logger3 = Logger.getLogger();
                logger3.mo24918d("Result was " + code);
                return ResponseParser.parse(code) == 0;
            } catch (IOException e) {
                Logger.getLogger().mo24921e("HTTP request failed.", e);
