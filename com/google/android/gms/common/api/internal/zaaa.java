...
final class zaaa implements zaca {
    ...
    private zaaa(Context context, zabe zabe, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map<C1048Api.AnyClientKey<?>, C1048Api.Client> map, Map<C1048Api.AnyClientKey<?>, C1048Api.Client> map2, ClientSettings clientSettings, C1048Api.AbstractClientBuilder<? extends zae, SignInOptions> abstractClientBuilder, C1048Api.Client client, ArrayList<zat> arrayList, ArrayList<zat> arrayList2, Map<C1048Api<?>, Boolean> map3, Map<C1048Api<?>, Boolean> map4) {
        this.zaa = context;
        this.zab = zabe;
        this.zam = lock;
        this.zac = looper;
        ...
        zabi zabi = r3;
        zabi zabi2 = new zabi(context2, zabe2, lock2, looper2, googleApiAvailabilityLight2, map2, (ClientSettings) null, map4, (C1048Api.AbstractClientBuilder<? extends zae, SignInOptions>) null, arrayList2, new zax(this, (zaw) null));
        this.zad = zabi;
        ...
        for (C1048Api.AnyClientKey<?> put : map2.keySet()) {
            arrayMap.put(put, this.zad);
        }
        for (C1048Api.AnyClientKey<?> put2 : map.keySet()) {
    ...
    private final boolean zaD(BaseImplementation.ApiMethodImpl<? extends Result, ? extends C1048Api.AnyClient> apiMethodImpl) {
    ...
    public static zaaa zag(Context context, zabe zabe, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map<C1048Api.AnyClientKey<?>, C1048Api.Client> map, ClientSettings clientSettings, Map<C1048Api<?>, Boolean> map2, C1048Api.AbstractClientBuilder<? extends zae, SignInOptions> abstractClientBuilder, ArrayList<zat> arrayList) {
        Map<C1048Api<?>, Boolean> map3 = map2;
        ArrayMap arrayMap = new ArrayMap();
        ArrayMap arrayMap2 = new ArrayMap();
        C1048Api.Client client = null;
        for (Map.Entry next : map.entrySet()) {
            C1048Api.Client client2 = (C1048Api.Client) next.getValue();
            if (true == client2.providesSignIn()) {
                client = client2;
            }
            if (client2.requiresSignIn()) {
                arrayMap.put((C1048Api.AnyClientKey) next.getKey(), client2);
            } else {
                arrayMap2.put((C1048Api.AnyClientKey) next.getKey(), client2);
            }
        }
        Preconditions.checkState(!arrayMap.isEmpty(), "CompositeGoogleApiClient should not be used without any APIs that require sign-in.");
        ArrayMap arrayMap3 = new ArrayMap();
        ArrayMap arrayMap4 = new ArrayMap();
        for (C1048Api next2 : map2.keySet()) {
            C1048Api.AnyClientKey<?> zab2 = next2.zab();
    ...
    public final ConnectionResult zad(C1048Api<?> api) {
    ...
    public final <A extends C1048Api.AnyClient, R extends Result, T extends BaseImplementation.ApiMethodImpl<R, A>> T zae(T t) {
        if (!zaD(t)) {
            this.zad.zae(t);
            return t;
        } else if (zaC()) {
    ...
    public final <A extends C1048Api.AnyClient, T extends BaseImplementation.ApiMethodImpl<? extends Result, A>> T zaf(T t) {
        if (!zaD(t)) {
