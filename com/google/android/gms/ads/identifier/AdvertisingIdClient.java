...
public class AdvertisingIdClient {
    ...
    private static zze zza(Context context, BlockingServiceConnection blockingServiceConnection) throws IOException {
        try {
            return zzf.zza(blockingServiceConnection.getServiceWithTimeout(Constants.VALID_WIFI_CONNECTION_CHECK_TIMEOUT, TimeUnit.MILLISECONDS));
        } catch (InterruptedException unused) {
    ...
    private final boolean zza(Info info2, boolean z, float f, long j, String str, Throwable th) {
        ...
        String str2 = "1";
        hashMap.put("app_context", z ? str2 : StatusCode.BUILTIN_WIRELESS);
        if (info2 != null) {
            if (!info2.isLimitAdTrackingEnabled()) {
                str2 = StatusCode.BUILTIN_WIRELESS;
            }
    ...
    public Info getInfo() throws IOException {
        ...
        synchronized (this) {
            ...
            Preconditions.checkNotNull(this.zze);
            Preconditions.checkNotNull(this.zzf);
            info2 = new Info(this.zzf.getId(), this.zzf.zzb(true));
        }
        zza();
        return info2;
    }
