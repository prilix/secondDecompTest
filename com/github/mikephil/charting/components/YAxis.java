...
public class YAxis extends AxisBase {
    ...
    public void setZeroLineWidth(float f) {
        this.mZeroLineWidth = C1030Utils.convertDpToPixel(f);
    }

    public float getRequiredWidthSpace(Paint paint) {
        paint.setTextSize(this.mTextSize);
        float calcTextWidth = ((float) C1030Utils.calcTextWidth(paint, getLongestLabel())) + (getXOffset() * 2.0f);
        float minWidth = getMinWidth();
        float maxWidth = getMaxWidth();
        if (minWidth > 0.0f) {
            minWidth = C1030Utils.convertDpToPixel(minWidth);
        }
        if (maxWidth > 0.0f && maxWidth != Float.POSITIVE_INFINITY) {
            maxWidth = C1030Utils.convertDpToPixel(maxWidth);
        }
        if (((double) maxWidth) <= C1030Utils.DOUBLE_EPSILON) {
    ...
    public float getRequiredHeightSpace(Paint paint) {
        paint.setTextSize(this.mTextSize);
        return ((float) C1030Utils.calcTextHeight(paint, getLongestLabel())) + (getYOffset() * 2.0f);
    }
