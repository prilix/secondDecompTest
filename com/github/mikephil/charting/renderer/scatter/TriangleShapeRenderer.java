...
public class TriangleShapeRenderer implements IShapeRenderer {
    ...
    public void renderShape(Canvas canvas, IScatterDataSet iScatterDataSet, ViewPortHandler viewPortHandler, float f, float f2, Paint paint) {
        ...
        float convertDpToPixel = (scatterShapeSize - (C1030Utils.convertDpToPixel(iScatterDataSet.getScatterShapeHoleRadius()) * 2.0f)) / 2.0f;
        ...
        float f7 = f - f3;
        path.lineTo(f7, f6);
        int i = (((double) scatterShapeSize) > C1030Utils.DOUBLE_EPSILON ? 1 : (((double) scatterShapeSize) == C1030Utils.DOUBLE_EPSILON ? 0 : -1));
        if (i > 0) {
            path.lineTo(f, f4);
