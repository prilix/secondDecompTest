...
public class YAxisRendererRadarChart extends YAxisRenderer {
    ...
    public void computeAxisValues(float f, float f2) {
        ...
        if (labelCount == 0 || abs <= C1030Utils.DOUBLE_EPSILON || Double.isInfinite(abs)) {
            this.mAxis.mEntries = new float[0];
            this.mAxis.mCenteredEntries = new float[0];
            this.mAxis.mEntryCount = 0;
            return;
        }
        double roundToNextSignificant = (double) C1030Utils.roundToNextSignificant(abs / ((double) labelCount));
        ...
        double roundToNextSignificant2 = (double) C1030Utils.roundToNextSignificant(Math.pow(10.0d, (double) ((int) Math.log10(roundToNextSignificant))));
        ...
        if (this.mAxis.isForceLabelsEnabled()) {
            ...
            int i2 = (roundToNextSignificant > C1030Utils.DOUBLE_EPSILON ? 1 : (roundToNextSignificant == C1030Utils.DOUBLE_EPSILON ? 0 : -1));
            if (i2 == 0) {
            ...
            if (i2 == 0) {
                d2 = C1030Utils.DOUBLE_EPSILON;
            } else {
                d2 = C1030Utils.nextUp(Math.floor(((double) f4) / roundToNextSignificant) * roundToNextSignificant);
            }
            if (i2 != 0) {
            ...
            for (int i4 = 0; i4 < i3; i4++) {
                if (d == C1030Utils.DOUBLE_EPSILON) {
                    d = 0.0d;
                }
    ...
    public void renderAxisLabels(Canvas canvas) {
        if (this.mYAxis.isEnabled() && this.mYAxis.isDrawLabelsEnabled()) {
            ...
            for (int i2 = !this.mYAxis.isDrawBottomYLabelEntryEnabled(); i2 < i; i2++) {
                C1030Utils.getPosition(centerOffsets, (this.mYAxis.mEntries[i2] - this.mYAxis.mAxisMinimum) * factor, this.mChart.getRotationAngle(), instance);
                canvas.drawText(this.mYAxis.getFormattedLabel(i2), instance.f195x + 10.0f, instance.f196y, this.mAxisLabelPaint);
            }
            ...
            for (int i = 0; i < limitLines.size(); i++) {
                ...
                if (limitLine.isEnabled()) {
                    ...
                    for (int i2 = 0; i2 < ((IRadarDataSet) ((RadarData) this.mChart.getData()).getMaxEntryCountSet()).getEntryCount(); i2++) {
                        C1030Utils.getPosition(centerOffsets, limit, (((float) i2) * sliceAngle) + this.mChart.getRotationAngle(), instance);
                        if (i2 == 0) {
