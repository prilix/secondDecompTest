...
public class Draft_6455 extends Draft {
    ...
    public HandshakeState acceptHandshakeAsServer(ClientHandshake clientHandshake) throws InvalidHandshakeException {
        ...
        while (true) {
            ...
            if (next.acceptProvidedExtensionAsServer(fieldValue)) {
                ...
                log.trace("acceptHandshakeAsServer - Matching extension found: {}", (Object) this.extension);
                break;
            }
        }
    ...
    private HandshakeState containsRequestedProtocol(String str) {
        for (IProtocol next : this.knownProtocols) {
            if (next.acceptProvidedProtocol(str)) {
                this.protocol = next;
                log.trace("acceptHandshake - Matching protocol found: {}", (Object) next);
                return HandshakeState.MATCHED;
            }
        }
        return HandshakeState.NOT_MATCHED;
        ...
        if (!basicAccept(serverHandshake)) {
            ...
            while (true) {
                ...
                if (next.acceptProvidedExtensionAsClient(fieldValue)) {
                    this.extension = next;
                    handshakeState = HandshakeState.MATCHED;
                    log.trace("acceptHandshakeAsClient - Matching extension found: {}", (Object) this.extension);
                    break;
                }
            }
    ...
    public ByteBuffer createBinaryFrame(Framedata framedata) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("afterEnconding({}): {}", (Object) Integer.valueOf(framedata.getPayloadData().remaining()), (Object) framedata.getPayloadData().remaining() > 1000 ? "too big to display" : new String(framedata.getPayloadData().array()));
        }
    ...
    private Framedata translateSingleFrame(ByteBuffer byteBuffer) throws IncompleteException, InvalidDataException {
        if (byteBuffer != null) {
            ...
            Logger logger = log;
            if (logger.isTraceEnabled()) {
                logger.trace("afterDecoding({}): {}", (Object) Integer.valueOf(framedataImpl1.getPayloadData().remaining()), (Object) framedataImpl1.getPayloadData().remaining() > 1000 ? "too big to display" : new String(framedataImpl1.getPayloadData().array()));
            }
    ...
    private void translateSingleFrameCheckLengthLimit(long j) throws LimitExceededException {
        if (j <= 2147483647L) {
            ...
            if (j > ((long) i)) {
                log.trace("Payload limit reached. Allowed: {} Current: {}", (Object) Integer.valueOf(i), (Object) Long.valueOf(j));
    ...
    private void checkBufferLimit() throws LimitExceededException {
        ...
        if (byteBufferListSize > ((long) this.maxFrameSize)) {
            clearBufferList();
            log.trace("Payload limit reached. Allowed: {} Current: {}", (Object) Integer.valueOf(this.maxFrameSize), (Object) Long.valueOf(byteBufferListSize));
