...
public class WebSocketImpl implements WebSocket {
    ...
    public void decode(ByteBuffer byteBuffer) {
        log.trace("process({}): ({})", (Object) Integer.valueOf(byteBuffer.remaining()), (Object) byteBuffer.remaining() > 1000 ? "too big to display" : new String(byteBuffer.array(), byteBuffer.position(), byteBuffer.remaining()));
    ...
    private boolean decodeHandshake(ByteBuffer byteBuffer) {
        ...
        try {
                        ...
                        try {
                            ...
                            if (copyInstance2.acceptHandshakeAsServer(clientHandshake) == HandshakeState.MATCHED) {
                                ...
                                try {
                                    ...
                                    log.trace("Closing due to wrong handshake. Possible handshake rejection", (Throwable) e);
                                    closeConnectionDueToWrongHandshake(e);
                                    return false;
                                } catch (RuntimeException e2) {
                                    log.error("Closing due to internal server error", (Throwable) e2);
            ...
            if (this.role == Role.CLIENT) {
                ...
                if (this.draft.acceptHandshakeAsClient(this.handshakerequest, serverHandshake) == HandshakeState.MATCHED) {
                    try {
                        ...
                        log.trace("Closing due to invalid data exception. Possible handshake rejection", (Throwable) e3);
                        flushAndClose(e3.getCloseCode(), e3.getMessage(), false);
                        return false;
                    } catch (RuntimeException e4) {
                        log.error("Closing since client was never connected", (Throwable) e4);
                        this.wsl.onWebsocketError(this, e4);
                        flushAndClose(-1, e4.getMessage(), false);
                        return false;
                    }
                } else {
                    log.trace("Closing due to protocol error: draft {} refuses handshake", (Object) this.draft);
            ...
            try {
                log.trace("Closing due to invalid handshake", (Throwable) e5);
    ...
    private void decodeFrames(ByteBuffer byteBuffer) {
        try {
            for (Framedata next : this.draft.translateFrame(byteBuffer)) {
                log.trace("matched frame: {}", (Object) next);
    ...
    public synchronized void close(int r6, java.lang.String r7, boolean r8) {
        ...
        L_0x0051:
            r1 = move-exception
            org.slf4j.Logger r3 = log     // Catch:{ all -> 0x0086 }
    ...
    private void send(Collection<Framedata> collection) {
        if (!isOpen()) {
            ...
            for (Framedata next : collection) {
                log.trace("send frame: {}", (Object) next);
    ...
    private void write(ByteBuffer byteBuffer) {
        log.trace("write({}): {}", (Object) Integer.valueOf(byteBuffer.remaining()), (Object) byteBuffer.remaining() > 1000 ? "too big to display" : new String(byteBuffer.array()));
    ...
    private void open(Handshakedata handshakedata) {
        log.trace("open using draft: {}", (Object) this.draft);
